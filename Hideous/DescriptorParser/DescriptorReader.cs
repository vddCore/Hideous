...
internal class DescriptorReader
{
    ...
    private readonly List<CollectionItem> _topLevelCollections = new();
    private readonly Stack<CollectionItem> _collectionDefinitionStack = new();

    private readonly Stack<Dictionary<GlobalTag, Item>> _globalsStack = new();

    private readonly List<Item> _locals = new();

    private Dictionary<GlobalTag, Item> Globals => _globalsStack.Peek();

    private CollectionItem? CurrentCollection => 
        _collectionDefinitionStack.TryPeek(out var collection) 
            ? collection
            : null;
